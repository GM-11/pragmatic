...
export default function Index() {
  ...
  const { isAuthenticated, isProSubscriber } = useAuthContext();
  ...
  const generateWallpaper = async (promptText: string) => {
    ...
    setLoading(true);
    try {
      // Check if user has reached the free tier limit (3 generations per day)
      if (!isAuthenticated) {
        Alert.alert(
          "Free Tier Limit",
          "Please sign in to generate more wallpapers.",
          [
            { text: "Cancel", style: "cancel" },
            {
              text: "Sign In",
              onPress: () => {
                // Navigate to settings tab for sign in
  ...
  const handleEditImage = async (editInstructions: string) => {
    if (!isProSubscriber) {
      Alert.alert(
        "Pro Feature",
        "Image editing is a pro feature. Upgrade to access premium features.",
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "Upgrade",
          ...
          <View className="pb-6 pt-3 space-y-3">
            <View className="bg-gray-50 rounded-2xl p-3 shadow-sm border border-gray-100">
              <TextInput
                ...
                placeholder={
                  isAuthenticated
                    ? "Describe your perfect wallpaper in detail..."
            ...
            <TouchableOpacity
              className={`${
                isAuthenticated ? "bg-rose-600" : "bg-gray-400"
              } p-4 rounded-xl w-full shadow-sm flex-row justify-center items-center flex mt-2`}
